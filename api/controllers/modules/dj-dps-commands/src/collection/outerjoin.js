...
module.exports = {
    name: "collection.outerjoin",
    synonims: {
        "collection.outerjoin": "collection.outerjoin",
        "collection.outerJoin": "collection.outerjoin",
        "c.outerjoin": "collection.outerjoin",
        "c.outerJoin": "collection.outerjoin" 
    },
    ...
    execute: function(command, state, config) {
        ...
        try {
           ...
           command.settings.collection.forEach( d => {
             let left_parts = state.head.data.filter( r => command.settings.criteria(d,r))
             if(left_parts.length == 0) {
                result.push({left:null,right:d})
             }
           })
