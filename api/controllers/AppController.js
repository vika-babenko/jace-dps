 ...
 */
var fs = require('fs');
// console.log("CREATE APP CONTROLLER")
...
module.exports = {
  ...
  createWithConfig: function (req, res) {
    // console.log("CREATE APP", JSON.stringify(req.param('config'),null,"\t"))
    ...
    var newAppConfig = _.cloneDeep(req.param('config'));
    

    newApp = Object.assign(newApp, newAppConfig)

    // console.log("STORE", JSON.stringify(newApp, null,"\t"))
        ...
        fs.readFile(".tmp/uploads/"+file.fd, function (err, body) {
          ...
          try {
            ...
            AppConfig.create(app).then(function (created) {
              ...
              sails.log.warn('AppController.import error: ' + err);
              res.serverError();
            });
          } catch (e) {
