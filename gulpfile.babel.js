...
const sourceLibDir = sourceDir+"/lib/**";
const sourceModuleDir = "./_modules/node_modules/**";
const sourceStaticModuleDir = "./_static_modules/**";
const sourceDjvueDir = sourceDir+'/djvue/**';
const sourceStaticResourcesDir = [
  sourceDir+'/img/**',
  sourceDir+'/data/**',
...
const buildDir = '.tmp/public';
const buildLibDir = buildDir+'/modules'
const buildModuleDir = buildDir+'/modules'
const buildDjvueDir = buildDir+'/djvue';
...
gulp.task("front-end-build", () => {
  plugins.run('cd front-end && node build/build.js' )
  .exec(
    () => {
      gulp.src(`${distDir}/**`)
        .pipe(plugins.cached('front-end-build'))
        .pipe(plugins.if(showFilesLog, plugins.size({showFiles: true, title: 'DELPOY FRONT-END'})))
        .pipe(gulp.dest(buildDir))
      gulp.src(`${distDir}/app.ejs`)
        .pipe(plugins.cached('front-end-build'))
        .pipe(plugins.if(showFilesLog, plugins.size({showFiles: true, title: 'DELPOY View'})))
...
gulp.task('copy-djvue', () => {
    gulp.src(sourceDjvueDir)
      .pipe(plugins.cached('copy-djvue'))
      .pipe(plugins.changed(buildDjvueDir))
      .pipe(plugins.if(showFilesLog, plugins.size({showFiles: true, title: 'DJVUE COMPONENT'})))
      .pipe(gulp.dest(buildDjvueDir))
  }
);
...
gulp.task('copy-modules', 
    ...
    gulp.src(sourceModuleDir)
      ...
      .pipe(plugins.if(showFilesLog, plugins.size({showFiles: true, title: 'MODULE'})))
      .pipe(gulp.dest(buildModuleDir))
  }
);

gulp.task('copy-static-modules', () => {
    gulp.src(sourceStaticModuleDir)
      .pipe(plugins.cached('copy-static-modules'))
      .pipe(plugins.changed(buildModuleDir))
      .pipe(plugins.if(showFilesLog, plugins.size({showFiles: true, title: 'STATIC MODULE'})))
      .pipe(gulp.dest(buildModuleDir))
  }
);
...
gulp.task('copy-libs', () => {
    gulp.src(sourceLibDir)
      .pipe(plugins.cached('copy-libs'))
      .pipe(plugins.changed(buildLibDir))
      .pipe(plugins.if(showFilesLog, plugins.size({showFiles: true, title: 'LIB'})))
      .pipe(gulp.dest(buildLibDir))
  }
);

gulp.task('copy-static-files', () =>
  gulp.src(sourceStaticResourcesDir, {base: 'assets'})
    .pipe(plugins.cached('copy-static-files'))
    .pipe(plugins.if(showFilesLog, plugins.size({showFiles: true, title: 'STATIC'})))
    .pipe(gulp.dest(buildDir))
);


// gulp.task('watch', ['build'], () =>
//     gulp.watch([watchDir, sourceModuleDir], ['copy-djvue','copy-modules','copy-static-files','copy-libs'])
//   );


gulp.task('watch', ['build'], () => {
    let c1 = gulp.watch(['assets/**'], ["copy-static-files", 'copy-libs', 'copy-djvue'])
    let c2 = gulp.watch(['front-end/src/**','front-end/public/**'], () => {
      plugins.run('cd front-end && node build/build.js' )
      .exec(
        () => {
          gulp.src(`${distDir}/**`)
